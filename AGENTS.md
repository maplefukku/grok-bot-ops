# AGENTS.md

ここは Grok Bot 工場の司令室です。プロダクトのコードはありません。あなたがここで扱うのは、routine の原稿、プロダクトの台帳、評価の記録、受理された教訓です。

## 読む順番

1. [`README.md`](./README.md) — 3 層の工場と、何がどこに住むか
2. [`docs/guide/README.md`](./docs/guide/README.md) — pstack の使い方（日本語訳）
3. プロダクトを触るなら [`products/`](./products/README.md) の該当エントリ

## 作業の原則

- 入口は `/poteto-mode`。スキルを列挙してマイクロマネジメントしない。
- テストコマンドは `scripts/quiet-test.sh -- <cmd>` を通す。quiet は skip ではない。
- ゴールと、合否が確認できる完了条件を先に書く。時間の長さは完了条件ではない。
- ビルドが通ったことは証拠にしない。本物のコマンド、画面、保存された値を取る。
- 判断は `/show-me-your-work` のログに残す。差分より判断を監査する。
- PR 本文の 4 見出しは [`docs/process/pr-body.md`](./docs/process/pr-body.md) である。pstack `/show-me-your-work` の TSV ではない。
- CI 梯子は [`docs/process/ci-ladder.md`](./docs/process/ci-ladder.md) である。Flag と E2E は FULL tip だけである。
- プロダクトのコード変更はこのリポジトリでやらない。該当プロダクトのリポジトリで cloud agent を立てる。

## このリポジトリへの書き込みルール

| 場所 | 書いてよいとき |
|---|---|
| `docs/knowhow/` | Grok Bot のアップデート・活用法を見つけたとき。出典リンクと日付が必須。人でなくトピックで分ける |
| `routines/` | routine の新設・変更。貼り付け前の原稿がここの正本 |
| `products/` | プロダクトの登録・状態更新（検証スキルを立てた、routine を付けた、など） |
| `evals/` | スキルやプロンプト変更を盲検評価したとき。結果を必ず残す |
| `lessons/` | `/reflect` を回し、人間が受理した教訓だけ |
| `skills/` | `evals/` のゲートを通ったスキルだけ。1 スキル 1 PR |
| `docs/guide/` | 原文（pstack 公式ガイド）の更新に追従するときだけ |
| `docs/decisions/` | ADR を採択・追加するとき。trend-log は Planner の採否台帳。毎日の CA は `ops/daily-YYYY-MM-DD` 上でのみ追記する |
| `docs/intent-memory/` | Intent/Memory の schema と読み方を変えるとき。感情をプロダクト DB に置かない |
| `scripts/intent_memory/` | Intent/Memory の query contract と隔離テストを変えるとき |
| `docs/mini-ops/` | Mini運用の常設手順。ワンショット掃除の記録は置かない |
| `docs/process/` | 出荷単位と ADV closer、lane、平日 cadence、PR-body Flag Y、CI 梯子 LIGHT→FULL、overnight /goal brief の手順、owner 席 shared-computer チェック。契約 SoT は置かない。毎日の CA はここへ書かない |

推測で箱を増やさないこと。必要になった瞬間に作ればよい。

## draft PR のマージ

毎日の collect / maintain は **draft PR を積むだけ**。ボットはマージしない。

- 見る人: このリポジトリの人間オーナー。毎日、開いている draft を見る。まとめてでよい。
- マージしてよいのは人間だけ。CI が落ちている PR はマージしない。
- `docs/knowhow/` の `確認: 未` は、マージ前に人間が中身と出典リンクを見る。見たことを PR コメントに残す。未確認のままマージしてよいが、出典が公式に辿れない行は落とす。
- プロダクト側の verify スキル PR も人間がマージする。ボットは draft のまま。
