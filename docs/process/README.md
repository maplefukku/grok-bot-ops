...
- 指標は units merged / day である。PR 断片数でも bot 数でもない。日次の数え方の SoT は [`scripts/merge-velocity-day.sh`](../../scripts/merge-velocity-day.sh) である。box `/workspace/fleet-scripts/merge-count-jst.sh` は WRAP のままこのパスを呼ぶか、sibling WRAP のまま残してよい。
