...
- CodeQL は [starter-workflows の `codeql.yml`](https://github.com/actions/starter-workflows/blob/main/code-scanning/codeql.yml) である。公式の言語検出に合わせ、このリポジトリの実体は `python` と `actions` である。javascript-typescript は公式が no-source-code で fatal になるので置かない。
- Scorecard は report-only である。出典は [ossf/scorecard-action](https://github.com/ossf/scorecard-action) と [ossf/scorecard の workflow 例](https://github.com/ossf/scorecard/blob/main/.github/workflows/scorecard-analysis.yml) である。`publish_results` は公式の private 挙動どおり false である。公開にして tooling を通さない。
- Secret Scanning は Org+GHAS まで HOLD である。この PR には置かない。
