...
4 行が全部 true のときだけ merge-ok である。merge-ok は PR確認と Closer の事実行である。PdM Flag Y は [`pr-body.md`](./pr-body.md) の **1 述語**（fleet 4 見出し MUST + merge-ok 4 行すべて true + Flag 表 5 行すべて true）である。[#127](https://github.com/maplefukku/grok-bot-ops/issues/127)。Flag 行を 6 行目以降 invent しない。
...
FAIL exit contract の AI-readable bundle は同じ [`quiet-test.md`](./quiet-test.md)（[issue 114](https://github.com/maplefukku/grok-bot-ops/issues/114)）である。SUCCESS は minimal。FAIL は exit code と bounded tail と full-log path である。この WRAP は bundle を持たない。
...
FAIL exit contract の Domain WRAP は [issue 114](https://github.com/maplefukku/grok-bot-ops/issues/114) である。SUCCESS は minimal である。FAIL は AI-readable bundle である。
