...
| 2026-09-05 | Knowhow収集 | 調査用の第4ボットを立てる | https://github.com/maplefukku/grok-bot-ops/issues/19 | REJECT | 調査は最先端手法と Knowhow収集、判断は Planner、発火は PdM と開発リーダー。席は既にある。足す前に席を減らす。 | none | trend-adopt-reject.md WRAP LIVE REJECT terminal; https://github.com/maplefukku/grok-bot-ops/issues/117; CreateAgent NONE |
...
| 2026-09-05 | 最先端手法 | Hermes /goal overnight repo cleanup | https://x.com/Teknium/status/2095412050751332838 | ADOPT | 長い /goal 一発のほうが細切れ TODO よりリポ掃除に効く。overnight CA に合う。既存 Cloud開発の done-when / domain-unit と重なるが、overnight 掃除専用の形（done-criteria + touch scope + diff cap、micro-step 禁止）はまだスキルに無い。Copilot/Hermes 製品そのものは採用しない。パターンだけ。 | skill | docs/process/overnight-goal.md WRAP LIVE 6 見出し (done-when, touch scope, diff cap, mid-run verify, verifier Soft-HOLD); scripts/overnight_goal.py; sand-workflow:overnight-goal-cleanup + sand-workflow:cloud; https://github.com/maplefukku/grok-bot-ops/issues/89; CreateAgent NONE; invent=N |
...
| 2026-09-07 | Knowhow収集 | Official Bot Marketplace templates (~69) + Haggle procurement | https://x.ai/news/grok-bot-procurement | ADOPT | 公式 Marketplace（約69）と調達記事は template/share の一次。CreateAgent 前に Marketplace を見て KEEP/WRAP。export-bot-template は残す。並列の自前公開経路は作らない。Haggle 調達ボット自体の量産はしない。 | ops | marketplace-precheck WRAP LIVE survey-before-CreateAgent; export-bot-template KEEP; sand-workflow:cloud; https://github.com/maplefukku/grok-bot-ops/issues/92; CreateAgent NONE |
...
| 2026-09-10 | Knowhow収集 | Inline draft approve-before-send | https://x.com/bot/status/2097759948189106686 | ADOPT | send-on-behalf / HITL の Soft Flag STOP auto-post と同型。既存 hitl-draft-ready / send-on-behalf を WRAPし、出して前の inline approve を厚くする。新席は作らない。 | skill |  |
| 2026-09-10 | Knowhow収集 | One-click multi-account switch | https://x.com/bot/status/2097759952375029917 | ADOPT | personal/work 二重席の運用材料。CBO / create-agent-persona と identity 設計に WRAP。CreateAgent 乱立はしない。 | ops |  |
| 2026-09-10 | Knowhow収集 | Harness +10% included usage (up to +35% heavy) | https://x.com/bot/status/2097759955214590272 | ADOPT | thrift / 週次バケット再計算。cursor post-session audit と Cursor運用 Usage+Spending WRAP。新ダッシュは invent しない。 | ops |  |
| 2026-09-10 | Knowhow収集 | ChatGPT → Grok Bot webhook bridge | https://x.com/moritzkremb/status/2097621535955833166 | REJECT | 証拠不足: webhook→Bot 自動実行は Guardian proposal-only と anti-job に反しやすい。再浮上: proposal-only webhook→PM/監視 digest が LIVE かつ explicit go があるとき。橋渡し製品席は作らない。 | none |  |
| 2026-09-10 | Knowhow収集 | CA reply-same-agent (no second launch per nudge) | https://x.com/iPuneetSingh/status/2097453766635044890 | ADOPT | CA hygiene / PR 乱立防止。cloud skill の reply/同一CA HARD を WRAP。nudgeごとの再launch禁止を明文化。CreateAgent NONE。 | skill |  |
| 2026-09-10 | Knowhow収集 | Three separate usage pools (Build / Bot / CA) | https://x.com/justsomeguy2365/status/2097754146204193122 | ADOPT | コーディングを Build 側へ寄せ Bot 枠を守る thrift。Cursor運用 spend WRAP + box≠product-code HARD と一致。新プール製品は invent しない。 | ops |  |
