...
| [`disk運用`](./disk運用.md) | mini.disk。ONE JOBは fukku-mac-mini（ListMachines→machineId）で空き%/GiB+prune候補を平日~2回。CLIはMini運用同系。worktree 手順は bot 参照欄。IN:Buddy、CTO、PM、Mini運用 peer。AGENCY disk-am（20 9 * * 1-5）、disk-pm（20 18 * * 1-5）。独立ジョブは並列。3美徳（ボットにやらせる / 会議せずPRかフラグ / 結果はオーナー） | いいえ |
