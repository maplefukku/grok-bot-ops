...
| 名前 | 役割 | 回すまで動かない |
|---|---|---|
| [`監視`](./監視.md) | fleet.supervisor。PdMの上。フリートの停滞を見てPdMへJOB。sweepは平日08-22を2時間おき JST（0 8,10,12,14,16,18,20,22 * * 1-5）。@every 2h ではない。回すまで待たない。新機能PRはdraft可。実装はしない。CAは /poteto-mode 必須。対象リポに pstack プラグイン必須。独立ジョブは並列。3美徳（ボットにやらせる / 会議せずPRかフラグ / 結果はオーナー） | いいえ |
